...
import { BookOpen, Brain, Headphones, GraduationCap, BarChart3, Copy, Check, Layers } from 'lucide-react';
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="bg-primary/5 border-primary/20">
          <CardContent className="p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-xl sm:text-2xl font-bold text-primary">{reductionPercent}%</p>
              <p className="text-xs sm:text-sm text-muted-foreground">shorter</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-accent/5 border-accent/20">
          <CardContent className="p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-xl sm:text-2xl font-bold text-accent">{readingTimeSaved} min</p>
        ...
        <Card className="bg-warm/5 border-warm/20">
          <CardContent className="p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-xl sm:text-2xl font-bold text-warm">{result.keyTerms.length}</p>
              <p className="text-xs sm:text-sm text-muted-foreground">key terms</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <Badge className={difficultyColors[result.difficulty_level]}>{result.difficulty_level}</Badge>
          </div>
      ...
      <Tabs defaultValue="summary" className="w-full">
        <TabsList className="w-full grid grid-cols-4 h-11">
          <TabsTrigger value="summary" className="gap-1.5 text-xs sm:text-sm">
            <BookOpen className="w-4 h-4 hidden sm:block" />Summary
          </TabsTrigger>
          <TabsTrigger value="terms" className="gap-1.5 text-xs sm:text-sm">
            <Brain className="w-4 h-4 hidden sm:block" />Key Terms
          </TabsTrigger>
          <TabsTrigger value="listen" className="gap-1.5 text-xs sm:text-sm">
            <Headphones className="w-4 h-4 hidden sm:block" />Listen
          </TabsTrigger>
          <TabsTrigger value="quiz" className="gap-1.5 text-xs sm:text-sm">
            <GraduationCap className="w-4 h-4 hidden sm:block" />Quiz
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="terms">
          <Card>
            ...
            <CardContent>
              {flashcardMode ? (
                <FlashcardMode keyTerms={result.keyTerms} onClose={() => setFlashcardMode(false)} />
              ) : (
        ...
        <TabsContent value="listen">
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                <ReadAlongText text={result.summary} progress={audioProgress} isPlaying={audioPlaying} />
              </CardContent>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="quiz">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <GraduationCap className="w-5 h-5 text-primary" /> Knowledge Check
              </CardTitle>
            </CardHeader>
            <CardContent>
